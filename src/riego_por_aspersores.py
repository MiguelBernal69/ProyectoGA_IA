...
def algoritmo_genetico(base, altura, cantidad_aspersores, radio, generaciones, umbral_porcentaje, tamano_poblacion):
    ...
    visualizar_generacion(poblacion[0], radio, base, altura, area_total, "Inicial", 0, ax)
    ...
    for generacion in range(generaciones):
        ...
        visualizar_generacion(mejor_individuo_generacion, radio, base, altura, area_total, generacion+1, mejor_porcentaje, ax)
...
def visualizar_generacion(poblacion, radio, base, altura, area_total, generacion, porcentaje_regado, ax):
    ax.clear()
    ...
    ax.set_xticks(np.arange(0, base+1, 5))  # Escala en pasos de 5
    ax.set_yticks(np.arange(0, altura+1, 5))  # Escala en pasos de 5
...
def generar_valores_sensores(departamento):
    humedad_tierra = random.uniform(0, 100)
    temperatura_ambiente = random.uniform(0, 100)
...
def main():
    ...
    print("------------------------------------------")
    visualizar_generacion(ultima_poblacion, radio, base, altura, Polygon([(0, 0), (base, 0), (base, altura), (0, altura)]), "Final", mejor_porcentaje, plt.gca())
